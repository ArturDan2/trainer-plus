...
const ChartStripe = ({
  ...
  const onClickHandler = async () => {
    ...
    if (Object.keys(weightData).length === 1 && weightData[year].length === 1) {
      //if it's an only value left, prevent from deleting it
      alert("Wykres musi zawierać minimum jeden pomiar");
    } else if (weightData[year].length > 1) {
      //if it's not a last weight in certain year, remove stripe only
      ...
      //if it's a last stripe to delete in certain year, remove the year from database and change view for year before
      await updateDoc(docRef, {
  ...
  return (
    <div
      onClick={(e) => {
        e.preventDefault();
        if (affilation === "mentee-page") return;
        setIsVisible(!isVisible);
      }}
      className={`chart-stripe ${affilation} ${isRemoved} ${
        isVisible ? "active" : "not-active"
      }`}
      style={{ height: dynamicHeight + "%" }}
    >
      <p>{weight}</p>
      <div
        className={`hidden-info ${affilation}  ${
          isVisible ? "visible" : "hidden"
        }`}
      >
        <button
          onClick={onClickHandler}
          className={`delete-stripe ${affilation}`}
        >
