...
const BodyComposition = ({affilation, bodycompositionData, mentee, setMentee}) => {
  ...
  return (
    <div className={`body-composition-chart ${affilation}`}>
      <div className="heading">
        <h4>SKŁAD CIAŁA</h4>
        {bodycompositionData ? <ExpendableButton windowInsides={<UpdateBodyCompositionWindow mentee={mentee} setMentee={setMentee} bodycompositionData={bodycompositionData}/>}/> : <></>}
      </div>
      ...
      }) : <EnterInitialBodycompositionData setMentee={setMentee} affilation={affilation} bodycompositionState={bodycompositionData} ></EnterInitialBodycompositionData>}
    </div>
    </div>
  )
}
