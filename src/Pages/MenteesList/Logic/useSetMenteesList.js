...
function useGetMentees(limitstatement) {
  ...
  const searchMentees = async (searchquery) => {
    //searchquery are the words entered in text input in search component
    const setQuery = () => {
      switch (searchquery.length) {
        case 1:
          ...
          return query(
            menteesCollectionRef,
            where("searchdata", "array-contains-any", searchquery)
          );
        case 2:
          ...
          return query(
            menteesCollectionRef,
            where("fname", "==", searchquery[0]),
            where("lname", "==", searchquery[1])
          );
        default:
          return;
      }
  ...
  const paginateMentees = async (e) => {
    const firebasequery = query(
      ...
      orderBy("timestamp", "desc"),
      limit(limitstatement),
      startAfter(menteesList[menteesList.length - 1].timestamp)
    );
    const data = await getDocs(firebasequery);
    const mentees = data.docs.map((doc) => ({ ...doc.data(), id: doc.id }));
    if (
      mentees[0] === undefined ||
      menteesList.some((mentee) => mentee.id === mentees[0].id)
    ) {
