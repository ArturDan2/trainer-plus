...
const ChartStripe = ({setMantee, affilation, weightData, year, object, scaleRatio}) => {
    ...
    return (
        ...
        <div onClick={(e)=> {e.preventDefault(); if(affilation == "mantee-page") return;  setIsVisible(!isVisible); console.log(isVisible)}} className={`chart-stripe ${affilation} ${isRemoved} ${isVisible ? "active" : "not-active"}`} style={{height: (weight)/scaleRatio + "%"}}>
            <p>{weight}</p>
